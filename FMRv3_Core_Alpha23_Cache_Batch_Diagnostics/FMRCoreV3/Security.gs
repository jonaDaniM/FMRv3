...
function assertFieldUserFmrV3_(
  email
) {
  const user =
    getUserFmrV3_(
  ...
  if (
    !user.canFieldTransact
  ) {
    throw new Error(
      user.role +
      ' cannot perform Field transactions.'
    );
  }
...
function assertBackorderAdminFmrV3_(
  email
) {
  const user =
    getUserFmrV3_(
