...
function boundDatabaseFingerprintV3_(
  databaseId
) {
  const digest =
...
function inspectBoundIdentityAndEnvironmentV3() {
  const properties =
    PropertiesService
      .getScriptProperties();
  ...
  return {
    ...
    diagnostic:
      'BOUND_IDENTITY_AND_ENVIRONMENT_ALPHA23',
    ...
    identityPolicy:
      'ACTIVE_USER_THEN_EFFECTIVE_USER_UNCHANGED_PENDING_DIAGNOSTIC'
  };
}

function runBoundIdentityAndEnvironmentDiagnosticV3() {
  const output =
  ...
  if (!output.passed) {
    throw new Error(
      'Bound identity and environment diagnostic failed.'
    );
  }
